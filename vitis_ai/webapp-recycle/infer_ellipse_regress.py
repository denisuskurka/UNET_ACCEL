...
# 1. Force CPU usage (Universal fix for Vitis AI)
...
# Configuration
# ----------------------------
# Must match your training dimensions (likely 128 or 256)
IMAGE_HEIGHT = 256
IMAGE_WIDTH  = 256
...
def preprocess_image_dpu(image_path, height=IMAGE_HEIGHT, width=IMAGE_WIDTH):
    ...
    Returns: input_data (1, H, W, 1), original_shape, original_gray_image
    """
...
def draw_ellipse_on_image(image_np, params, color=(0, 255, 0)):
    ...
    """
    # Prepare image for drawing (Convert to BGR uint8)
...
def load_regressor_model(hw=False):
    global regressor_model_sw, dpu_runner, g_graph, dpu_input_ndim, dpu_output_ndim
    ...
    if hw:
        ...
        # compile=False allows loading without loss definitions
        regressor_model_sw = tf.keras.models.load_model(SW_MODEL_PATH, compile=False)
...
def infer_ellipse(hw=False):
    ...
    if hw:
        ...
        # Use the resized image for drawing the result
        ...
        # Extract Result
        # Output is (1, 5) -> [cx, cy, ax1, ax2, angle]
    ...
    # Draw ellipse on the 128x128 (or 256x256) image
